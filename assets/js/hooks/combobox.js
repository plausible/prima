...
export default {
  ...
  setupElements() {
    ...
    this.refs.createOption = this.refs.optionsContainer?.querySelector(
      SELECTORS.CREATE_OPTION,
    );
    this.refs.selectionTemplate = this.refs.selectionsContainer?.querySelector(
      SELECTORS.SELECTION_TEMPLATE,
    );

    const referenceSelector =
      this.refs.optionsWrapper?.getAttribute("data-reference");
    this.refs.referenceElement = referenceSelector
    ...
    if (this.refs.optionsContainer) {
      this.listeners.push(
        ...
        [
          ...
          "phx:show-start",
          this.handleShowStart.bind(this),
        ],
        [
          this.refs.optionsContainer,
          "phx:hide-end",
          this.handleHideEnd.bind(this),
        ],
      );
    }
  ...
  ensureOptionIds() {
    ...
    const options = this.refs.optionsContainer.querySelectorAll(
      SELECTORS.OPTION,
    );
  ...
  updated() {
    ...
    const focusedDomNode = this.refs.optionsContainer?.querySelector(
      `${SELECTORS.OPTION}[data-value="${this.focusedOptionBeforeUpdate}"]`,
    );
  ...
  getMode() {
    return this.refs.searchInput.hasAttribute("phx-change")
      ? "async"
      : "frontend";
  },
  ...
  getVisibleOptions() {
    return Array.from(
      this.refs.optionsContainer?.querySelectorAll(SELECTORS.VISIBLE_OPTION) ||
        [],
    );
  },
  ...
  getRegularOptions() {
    return (
      this.refs.optionsContainer?.querySelectorAll(SELECTORS.REGULAR_OPTION) ||
      []
    );
  },
  ...
  getSelectedValues() {
    const inputs =
      this.refs.submitContainer?.querySelectorAll('input[type="hidden"]') || [];
  ...
  setFocus(el) {
    this.refs.optionsContainer
      ?.querySelector(SELECTORS.FOCUSED_OPTION)
  ...
  focusFirstOption() {
    const firstOption = this.refs.optionsContainer?.querySelector(
      SELECTORS.VISIBLE_OPTION,
    );
    if (firstOption) {
  ...
  navigateUp(e) {
    ...
    const currentFocusIndex = visibleOptions.findIndex(
      (option) => option.getAttribute("data-focus") === "true",
    );
    const targetIndex =
      currentFocusIndex <= 0
  ...
  navigateDown(e) {
    ...
    const targetIndex =
      currentFocusIndex === visibleOptions.length - 1
        ? 0
        : currentFocusIndex + 1;
    this.setFocus(visibleOptions[targetIndex]);
  },
  ...
  handleInput(e) {
    ...
    if (
      !this.isMultiple &&
      searchValue === "" &&
      this.getSelectedValues().length > 0
    ) {
  ...
  handleAsyncMode() {
    ...
    this.focusedOptionBeforeUpdate =
      this.getCurrentFocusedOption()?.dataset.value;
  },
  ...
  positionOptions() {
    ...
    const placement =
      this.refs.optionsWrapper.getAttribute("data-placement") || "bottom-start";
    const shouldFlip =
      this.refs.optionsWrapper.getAttribute("data-flip") !== "false";
  ...
  showOptions() {
    ...
    this.liveSocket.execJS(
      this.refs.optionsContainer,
      this.refs.optionsContainer.getAttribute("js-show"),
    );

    this.refs.optionsContainer.addEventListener(
      "phx:show-end",
      () => {
        this.focusFirstOption();
      },
  ...
  hideOptions() {
    ...
    this.liveSocket.execJS(
      this.refs.optionsContainer,
      this.refs.optionsContainer.getAttribute("js-hide"),
    );
  ...
  setupClickOutsideHandler() {
    const handleClickOutside = (event) => {
      if (
        !this.refs.optionsContainer.contains(event.target) &&
  ...
  updateCreateOptionVisibility(searchValue) {
    ...
    if (searchValue.length > 0 && !this.hasExactMatch(searchValue)) {
      ...
      const createOptionHasFocus =
        this.refs.createOption.getAttribute("data-focus") === "true";
      this.hideOption(this.refs.createOption);
