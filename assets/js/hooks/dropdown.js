...
export default {
  ...
  handleTypeahead(e) {
    ...
    const currentIndex =
      currentFocused && matchingItems.includes(currentFocused)
        ? matchingItems.indexOf(currentFocused)
        : -1;

    const nextIndex =
      currentIndex >= 0 && currentIndex < matchingItems.length - 1
        ? currentIndex + 1
        : 0;
  ...
  clearFocus() {
    this.el
      .querySelector(SELECTORS.FOCUSED_MENUITEM)
  ...
  positionMenu() {
    ...
    const placement =
      this.refs.menuWrapper.getAttribute("data-placement") || "bottom-start";
    const shouldFlip =
      this.refs.menuWrapper.getAttribute("data-flip") !== "false";
